...
(function() {
  ...
  var cursor_offset = 0;
  var last_cursor = 0;
  const shared_timeout = 100;
  const shared_buffer_limit = 6291456;
  ...
  function init_ui(settings, emulator) {
    ...
    if (settings.id == 'winxp') {
      shared_offset = 0x02108020;
      mouse_offset = 0x020F0008;
      cursor_offset = 0x020143E4;
      file_buffer = [[
        'C:\\Documents and Settings\\Administrator\\Desktop\\drag_and_drop.txt',
        text_encoder.encode('Just Drag and Drop Files to Import them To The Emulator!'),
        true
      ]];
      last_cursor = 0;
      setInterval(check_cursor, 1000 / 10);
      setTimeout(process_files, 1000);
    }
    ...
    function process_files() {
      ...
      if (emulator.read_memory(shared_offset, 1)[0] !== 97) { // Already doing something...
        setTimeout(process_files, shared_timeout);
        return;
      }
      if (file_buffer[0][2]) { // Create File
        file_buffer[0][2] = false;
        emulator.write_memory(text_encoder.encode('b' + file_buffer[0][0] + '\x00'), shared_offset);
        setTimeout(process_files, shared_timeout);
        return;
      }
      if (file_buffer[0][1].length <= 0) { // Close File
        file_buffer.shift(0);
        emulator.write_memory(text_encoder.encode('d\x00'), shared_offset);
        setTimeout(process_files, shared_timeout);
        return;
      }
      // Write File Content
      if (file_buffer[0][1].length <= shared_buffer_limit - 10) {  // File Smaller than Buffer Size
        const buf = new Uint8Array(file_buffer[0][1].length + 10);
        buf.set(shared_str_size(file_buffer[0][1].length), 0);
        buf.set(file_buffer[0][1], 10);
        emulator.write_memory(buf, shared_offset);
        file_buffer[0][1] = new Uint8Array(0);
        setTimeout(process_files, shared_timeout);
        return;
      }
      setTimeout(process_files, shared_timeout);
